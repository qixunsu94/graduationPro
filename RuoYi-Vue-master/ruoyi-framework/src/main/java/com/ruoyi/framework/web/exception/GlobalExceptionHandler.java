...
public class GlobalExceptionHandler {
    ...
    public AjaxResult handleException(Exception e, HttpServletRequest request) {
        ...
        if (e instanceof IOException && e.getMessage() != null
                && (e.getMessage().contains("中止") || e.getMessage().contains("Broken pipe")
                        || e.getMessage().contains("Connection reset") || e.getMessage().contains("断开"))) {
            log.warn("请求地址'{}',客户端连接已断开: {}", requestURI, e.getMessage());
            return AjaxResult.error("客户端连接已断开");
        }
